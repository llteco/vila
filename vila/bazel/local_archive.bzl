"""Rules for extracting archives on local urls.

Usage is similar with `http_archive`.
"""

load("@bazel_tools//tools/build_defs/repo:utils.bzl", "patch", "workspace_and_buildfile")

def _local_archive_impl(ctx):
    ctx.extract(ctx.attr.src, stripPrefix = ctx.attr.strip_prefix)
...
Examples:
  ...
  which is available from @//external:openssl.zip. This `.zip` file
  ...
  ```python
  load("@//bazel:local_archive.bzl", "local_archive")

  local_archive(
      name = "my_ssl",
      src = "@//example:openssl.zip",

/************************************************************************
 * Copyright (C) 2023-2026 The CLIM Authors.
 *
...
/**
 * @brief Generate a tensor full of ones.
 *
 * @tparam Container: the STL container type
 * @tparam Shape: the shape STL container type
 * @param shape: the shape of the tensor
 * @return Container: tensor full of ones
 */
template <
    class Container,
    class Shape,
    must_be_int<typename Shape::value_type> = 0>
Container Ones(const Shape& shape) {
  using T = typename Container::value_type;
  Container a(PI(shape), static_cast<T>(1));
  return a;
}

/**
 * @brief Generate a tensor full of zeros.
 *
 * @tparam Container: the STL container type
 * @tparam Shape: the shape STL container type
 * @param shape: the shape of the tensor
 * @return Container: tensor full of zeros
 */
template <
    class Container,
    class Shape,
    must_be_int<typename Shape::value_type> = 0>
Container Zeros(const Shape& shape) {
  using T = typename Container::value_type;
  Container a(PI(shape), static_cast<T>(0));
  return a;
}

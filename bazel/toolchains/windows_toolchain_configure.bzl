...
def get_msvc_vars(repository_ctx, paths, target_arch = "x64", msvc_vars_x64 = None):
    r"""Get the variables we need to populate the MSVC toolchains.

    Args:
        repository_ctx: the repository_ctx object
        paths: a dictionary of paths to template files
        target_arch: the target architecture to get its environment variables
        msvc_vars_x64: values of MSVC toolchain including the environment variables for x64 target architecture

    Returns:
        a dictionary of variables for the MSVC toolchain
    """
    msvc_vars = dict()
...
def get_clang_cl_vars(repository_ctx, paths, msvc_vars, target_arch):
    r"""Get the variables we need to populate the clang-cl toolchains.

    Args:
        repository_ctx: the repository_ctx object
        paths: a dictionary of paths to template files
        msvc_vars: values of MSVC toolchain including the environment variables for x64 target architecture
        target_arch: the target architecture to get its environment variables

    Returns:
        a dictionary of variables for the clang-cl toolchain
    """
    llvm_path = find_llvm_path(repository_ctx)
    error_script = None
...
def get_path_env_var(repository_ctx, name):
    r"""Export this private function. Get the value of the PATH environment variable.
    ...
    """
    return _get_path_env_var(repository_ctx, name)
